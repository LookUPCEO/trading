...
- I.3 70% 방향쏠림 건수 ⬜ (다음 — 승부처. whitened 21차원 1.135M DB 에서 top-N 미래방향 쏠림):
  - ⚠️ 필수: 간극천장(causal 0.37bp) 대조, fee/fill/시기 audit, 진입 ≥ 라벨시각+1s, binomial 우연 대비, **매치 클러스터링** (같은 날 인접 분 = 독립 표 아님), 1~7일 exclusion zone 검토(비용 ≤1%).
- I.4 fee 넘는 폭 건수 ⬜
...
**마지막 업데이트**: 2026-06-06 (I.2+ 전체기간 확장 ✅ — 1198일 1.44M행 라벨링(lookahead 0 재확인), 203일과 45/45 동일, 축약 21차원 재현, DB 1.135M행(6배), 부호일치 0.798. 시기 골고루·직근 쏠림 없음. 다음 I.3 70% 쏠림(승부처) — 간극천장 0.37bp·매치 클러스터링·fee/fill/시기 audit 필수. 같은 날: I.1+ 정확성 보강 ✅(버그3 수정), I.2 ✅)
