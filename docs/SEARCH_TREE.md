...
**마지막 업데이트**: 2026-06-16 (I.21 DTW ❌ — 새 정보(겹침 0.37)이나 방향 비정렬(DTW hit 0.571 ≤ 유클리드 0.585, test 음수). 모양 정보 3회 확인(8 redundant/18 희석/21 비정렬) — 시간적 모양은 방향 예측에 순간 21차원 못 이김. 4h 유클리드 순간상태 확정, 신호 표현 탐색 종결. 빈도+거래당+표현 모두 소진, 키우는 길=레버리지(16). 감쇠 판정(shadow) 선결. 이전: I.20 1d 기각)
