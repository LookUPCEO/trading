...
- I.6 ⬜ (남은 형제들 — 우선순위 갱신):
  - **shadow/paper 전향 검증** (최우선 — 2025Q3+ 미확정의 유일하게 정직한 해소)
  - **능동 청산** (고정 hold → trailing/부분; E.2 연결, 일수익 15bp 개선 후보)
...
**마지막 업데이트**: 2026-06-07 (I.6a 닮음 정의 31개 ❌ — train 상위 V/VOC/O 전부 OOS 사망 (V -36, O -57), 21차원 전체가 유일 생존 = 유지 확정. 사전등록 분리가 과적합 차단 실증. 빈도 개선 실패 (부분공간 = 이벤트 희소화). 다음: shadow 전향검증 최우선. 이전: I.5 4h thr70 Bonf 단독생존)
