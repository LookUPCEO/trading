...
**Last updated:** 2026-06-28 (I.34 21 vs 31 충돌 해소 — 31 맞음, 21 과대평가(소표본). 같은 신호 세 측정: stage5 stride-10 n=79 hit 0.684/CI 0제외/5폴드 = 소표본 행운, 11배 표본 dense서 hit 0.556/CI 0포함/3폴드 무너짐, 라이브 earliest 본전. 0.084% 과대, 진짜≈본전(hit 0.52~0.56 약우위·fee 상쇄). shadow 우호적=무dedup 모집단. 본인 의심 정당. 운영점 불변, 유일 판정=실측) // 이전: I.33 탄력적 진입 ❌) — 수수료 아니라 "갈아탈 강신호 부재"가 벽: 0.70 진입 후 hold 중 fup≥0.85 도달 0%(225중 1), kNN ~0.78 천장. 갈아탄 극소·OOS 0 검증불가. 31 본전 그대로, 운영점 불변. 본인 의도 정확히 봤으나 강신호 부재로 못 풂) // 이전: I.32 31 재검증) — 31 본질 맞음, "≈0"→"방향 54% 맞지만 fee 와 정확히 상쇄=본전, 방향우위 실재". 데몬fup=백테fup corr 0.961(모순해소, shadow 우호적=경계노이즈+소표본 행운). 라이브 백테처럼 거래 불가(+90.5=행운/레버리지). causal 진입 다 해봄=earliest 못 이김. 레버리지=위험만. 유일 판정=실측) // 이전: I.31확장 fup 강도별 ❌ — earliest 진입 fup max 0.762 갇힘(강fup 도달 불가), 진입 thr↑는 표본붕괴 OOS검증불가. fup↑→방향+크기↑는 in-sample 실재이나 OOS 표본0. 레버리지·선별 둘 다 marginal 극복 실패. 거래가능·OOS edge ≈0, 유일 판정=실측) // 이전: I.31 earliest-crossing 재검토 — 0.084%/+90.5 헤드라인 상당부분 측정 인공물(stride-10 91분위 행운+비거래가능 mid-run). 라이브 실제 OOS=+0.26bp/day CI[−34,+74]=0과 구분 불가. [I] 진짜 edge 미확립. 레버리지 흔들림. 유일 판정=실측. 죽음 단정은 X) // 이전: I.30 롱전용 vs 롱+숏 — 숏=음수EV 데드웨이트(6단계 정합), 롱 전용 권고(거래당↑·구현 단순). 정직: 라이브 진입방식선 롱도 marginal — 헤드라인 +90.5는 stride/표본 산물, 최종판정 라이브/shadow. 라이브 숏게이트 확인 대기)
...
## 🔬 2026-06-28 — I.34: 21(진짜) vs 31(환상) 충돌 해소 — 31 맞음, 21 과대평가(소표본)

- 같은 4h thr0.70 신호 세 측정: **A stride-10 무dedup(stage5 원본) n=79 hit 0.684 day+74.7 CI[+28,+123] 0제외 5/5폴드** / **B dense 무dedup(매분 11배) hit 0.556 +13.7 CI 0포함 3/5** / **C dense earliest(라이브) hit 0.516 −6.5 CI 0포함 3/5**.
- **결정타**: stage5 "5/5·CI 0제외·hit 0.684"는 n=79 소표본 행운 — 같은 신호 11배 표본이면 hit 0.556·CI 0포함·폴드 3/5 로 무너짐. 21은 틀린 방법 아니라 소표본이 진짜 작은 edge 부풀림(Bonferroni 통과해도 n=79 자체 행운).
- **왜 21 동안 못 잡나**: 1~29 전부 stride-10+무dedup 일관 → 내부 모순 안 드러남. 30단계 "라이브 실제 진입(earliest)" 처음 물으니 노출.
- **shadow 모순 해소**: 기록데몬=B 무dedup(우호적), 라이브=C earliest(본전). shadow 우호적=B 모집단 소표본 → C와 모순 아님.
- **판정**: 0.084% 과대평가, 진짜≈본전(hit 0.52~0.56 약우위 실재·fee 상쇄·CI 0포함). 31 "환상"은 맥락 있음(측정을 라이브 실제로 바꾸니 드러남), "본전"이 "≈0 무작위"보다 정확. 본인 의심 정당. 운영점 불변, 유일 판정=실측.

---

## 🔬 2026-06-28 — I.33: 탄력적 진입 (강해지면 갈아타기) ❌

- 본인 의도 정확히: 6-1/6-2(나빠지면 청산)와 다른 "같은 방향 강해지면(fup↑) 청산+재진입(업그레이드)".
- **작업1(결판)**: 0.70 진입 후 4h hold 중 fup max 평균 0.726, **fup≥0.85 도달 225건 중 1건(0%)**, ≥0.78 7%. kNN 합의 ~0.78 천장(진입·hold 둘 다) — **갈아탈 +134 강영역 사실상 비어있음**.
- 작업2/3: 갈아타기(수수료 2배 22bp) 전체 −1.4 vs 고정 −4.1(약간↑) but 갈아탄 14/3/1 극소, **OOS 갈아탄 건 0개=검증불가(전략=고정)**.
- 작업4: 갈아탄 14건 −10.2 vs 같은건 고정 −53.9(나쁜 진입 덜 나쁘게)이나 음수+OOS0. 고점 재진입 되돌림. 피라미딩=one-way 평단가(레버리지).
- **판정**: 수수료 아니라 "갈아탈 강신호 부재"가 벽. 31 본전 그대로. 9(빈도붕괴)·31확장(강fup 도달불가) 같은 벽. 운영점 불변(고정 4h earliest). 유일 판정=실측.
