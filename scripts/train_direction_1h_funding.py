"""1-hour Direction + Funding Rate Extreme."""
...
def main():
    ...
    log.info("=" * 70)
    log.info("1H DIRECTION + FUNDING RATE EXTREME (시도 5)")
    log.info("=" * 70)
    ...
    if target_col not in train_df_full.columns:
        log.error(f"{target_col} not found")
        return

    funding_col = "dt_funding_rate"
    if funding_col not in train_df_full.columns:
        log.error(f"{funding_col} not found")
        return

    log.info(f"Using funding column: {funding_col}")

    f_stats = train_df_full[funding_col].describe()
    log.info(f"Funding stats: mean={f_stats['mean']:.6f}, std={f_stats['std']:.6f}")
    log.info(f"  abs quantiles: 50%={train_df_full[funding_col].abs().quantile(0.5):.6f}, 75%={train_df_full[funding_col].abs().quantile(0.75):.6f}, 90%={train_df_full[funding_col].abs().quantile(0.90):.6f}")

    for df in [train_df_full, val_df_full, test_df_full]:
        before = len(df)
        df.dropna(subset=[target_col, funding_col], inplace=True)
    ...
    train_funding_abs = train_df_full[funding_col].abs()
    f50 = train_funding_abs.quantile(0.50)
    f75 = train_funding_abs.quantile(0.75)
    f90 = train_funding_abs.quantile(0.90)

    train_funding = train_df_full[funding_col]
    fneg50 = train_funding.quantile(0.25)  # bottom 25% (most negative)
    fpos50 = train_funding.quantile(0.75)  # top 25% (most positive)

    log.info(f"Funding quantiles: |50%|={f50:.6f}, |75%|={f75:.6f}, |90%|={f90:.6f}")
    log.info(f"  neg 25%ile: {fneg50:.6f}, pos 75%ile: {fpos50:.6f}")

    T = 0.20

    VARIANTS = [
        ("1h T=0.20 (no funding)",                          0.0,    "none"),
        ("1h T=0.20 + |funding|>50%ile",                    f50,    "abs"),
        ("1h T=0.20 + |funding|>75%ile",                    f75,    "abs"),
        ("1h T=0.20 + |funding|>90%ile",                    f90,    "abs"),
        ("1h T=0.20 + funding<bot25%ile (long pressure)",   fneg50, "neg"),
        ("1h T=0.20 + funding>top25%ile (short pressure)",  fpos50, "pos"),
    ]
    ...
    for variant_name, f_thresh, direction in VARIANTS:
        print()
        print("=" * 80)
        ...
        def filter_df(df):
            ...
            if direction == "abs":
                mask &= df[funding_col].abs() >= f_thresh
            elif direction == "neg":
                mask &= df[funding_col] <= f_thresh
            elif direction == "pos":
                mask &= df[funding_col] >= f_thresh
            return df[mask].copy()
    ...
    print()
    print("=" * 110)
    print("FINAL COMPARISON: 1h Direction + Funding Rate")
    print("=" * 110)
    print()
    print(f"{'Variant':<48} {'Test n':<8} {'Retain':<10} {'LR AUC':<10} {'XGB AUC':<10} {'@val_thr':<10} {'iter':<8}")
    print("-" * 110)

    print(f"{'시도1: 5m T=0.20':<48} {'876':<8} {'12.2%':<10} {'0.580':<10} {'0.580':<10} {'-':<10} {'36':<8}")
    print(f"{'시도2: 5m + OBI>0.3':<48} {'661':<8} {'11.5%':<10} {'0.565':<10} {'0.611':<10} {'0.546':<10} {'105':<8}")
    print(f"{'시도3: 1h T=0.20':<48} {'3290':<8} {'59.6%':<10} {'0.610':<10} {'0.620':<10} {'-':<10} {'0':<8}")
    print()

    for name, r in all_results.items():
        print(f"{name:<48} {r['test_size']:<8} {r['test_retention']:<10.1f}% {r['lr_auc']:<10.3f} {r['xgb_auc']:<10.3f} {r['lr_test_bal_at_val']:<10.3f} {r['best_iter']:<8}")
    ...
    print("=" * 80)
    print()
    print("Funding rate 효과 평가:")
    print("  Abs (양/음 모두): trend 신호")
    print("  Neg (음의 funding): 숏 압력 강함 -> 가격 상승 가능 (squeeze)")
    print("  Pos (양의 funding): 롱 압력 강함 -> 가격 하락 가능 (reversal)")
    print()
    print("AUC 향상 시나리오:")
    print("  > 0.62 -> 시도 5 효과 미미")
    print("  > 0.65 -> 시도 5 성공")
    print("  > 0.68 -> 시도 5 큰 성공")
