"""mark36_v2 walk-forward backtest: n_estimators=100 fixed (no early_stop)."""
...
def main():
    ...
    log.info("=" * 70)
    log.info("mark36_v2 walk-forward BACKTEST (no early_stop)")
    log.info("=" * 70)
    np.random.seed(42)
    ...
    for step_idx, test_date in enumerate(test_dates, 1):
        idx = SELF_ALL.index(test_date)
        train_self_dates = SELF_ALL[:idx]
        log.info(f"\n=== STEP {step_idx}/9 test {test_date} ===")
        ...
        for dir_th in DIR_THS:
            ...
            n_sl = 0; n_maker = 0
            n_long_signals = 0; n_short_signals = 0
            while i < n:
                r = d_df.iloc[i]
                ...
                if r["vol_proba"] > VOL_TH:
                    if r["dir_proba"] > dir_th: direction = 1; trade = True; n_long_signals += 1
                    elif r["dir_proba"] < (1 - dir_th): direction = -1; trade = True; n_short_signals += 1
                if trade:
                    ...
                    if sl:
                        ...
                        if filled: n_maker += 1
                    trades.append({"net_pnl": ar - (FEE_TAKER + fee_e), "side": "L" if direction == 1 else "S"})
                    i += LOCKOUT
                else:
                    i += 1
            n_total = len(trades)
            ...
            n_long_filled = sum(1 for t in trades if t["side"] == "L")
            n_short_filled = sum(1 for t in trades if t["side"] == "S")
            per_th[str(dir_th)] = {"pnl": ps, "n_trades": n_total, "win_rate": wr,
                                    "n_sl": n_sl, "n_maker": n_maker,
                                    "n_long": n_long_filled, "n_short": n_short_filled}
            log.info(f"    TH {dir_th}: {ps:+.3f}% ({n_total}t L{n_long_filled}/S{n_short_filled}, win {wr*100:.1f}%, SL {n_sl}, maker {n_maker})")
    ...
    print()
    print("=" * 100)
    print("WALK-FORWARD BACKTEST mark36_v2 (n=100 fixed, no early_stop)")
    print("=" * 100)
    for th in DIR_THS:
        print(f"\nDIR_TH = {th}")
        print(f"{'Step':<6} {'Date':<14} {'AUC':<8} {'PnL':<10} {'L/S':<8} {'Trades':<8} {'Win':<8}")
        print("-" * 70)
        pnls = []
        for r in walk_results:
            t = r["per_th"][str(th)]
            print(f"{r['step']:<6} {r['test_date']:<14} {r['auc_self']:<8.3f} {t['pnl']:<+10.3f}% {t['n_long']}/{t['n_short']:<5} {t['n_trades']:<8} {t['win_rate']*100:<8.1f}%")
            pnls.append(t["pnl"])
        v = np.array(pnls); pos = (v > 0).sum()
        total_long = sum(r["per_th"][str(th)]["n_long"] for r in walk_results)
        total_short = sum(r["per_th"][str(th)]["n_short"] for r in walk_results)
        print(f"  Mean {v.mean():+.3f}%/day  Std {v.std():.3f}  Min {v.min():+.3f}  Max {v.max():+.3f}  Positive {pos}/9  Total {v.sum():+.3f}%  L:{total_long}/S:{total_short}")

    out = {"approach": "mark36_v2 (n=100 fixed) walk-forward backtest", "steps": walk_results}
    out_path = Path("/Users/dohun/Desktop/Mark/mark19/data/analysis_results/sido36v2_backtest.json")
    ...
    print(f"  vs sido36 v1 best (TH 0.55): +0.590%/day (9 days)")
    if best_mean >= 0.5:
        print(f"  ✅ mark36_v2 backtest robust (≥0.5%/day, 9 days)")
    elif best_mean >= 0:
        print(f"  🟡 mark36_v2 marginal positive")
    else:
        print(f"  ❌ mark36_v2 negative — overfit 영향 가능 (Train AUC 0.896 vs Val 0.576)")

    log.info("\nmark36_v2 backtest complete")
