"""Position Sizing Strategies (시도 15)."""
...
def main():
    ...
    log.info("=" * 70)
    log.info("POSITION SIZING (시도 15)")
    log.info("=" * 70)
    ...
    def size_fixed(dir_proba):
        return 1.0

    def size_linear(dir_proba):
        return min(abs(dir_proba - 0.5) * 4, 2.0)

    def size_quadratic(dir_proba):
        confidence = abs(dir_proba - 0.5) * 2
        return min(confidence ** 2 * 2.5, 2.5)

    def size_threshold(dir_proba):
        confidence = abs(dir_proba - 0.5)
        if confidence > 0.20:
            return 2.0
    ...
    def size_kelly(dir_proba):
        win_rate = 0.62
        avg_win = 0.40
        avg_loss = 0.40
        edge = win_rate * avg_win - (1 - win_rate) * avg_loss
        kelly_base = edge / (avg_win * avg_loss)
        signal = abs(dir_proba - 0.5) * 2
        return min(kelly_base * signal * 2, 2.5)

    SIZING_STRATEGIES = {
        "1_fixed (시도 6 baseline)": size_fixed,
        "2_linear": size_linear,
        "3_quadratic": size_quadratic,
        "4_threshold": size_threshold,
        "5_kelly": size_kelly,
    }
    ...
    print()
    print("=" * 80)
    print("STRATEGY × FEE COMPARISON")
    print("=" * 80)

    all_results = {}

    for sizing_name, sizing_fn in SIZING_STRATEGIES.items():
        for fee_name, fee_pct in FEE_SCENARIOS.items():
            ...
            for date_str in DATES_TEST:
                ...
                while i < n:
                    ...
                    if trade:
                        size = sizing_fn(dir_proba)
                        pnl = direction * actual * size - fee_pct * size
                        date_trades.append({
                            "ts": row[ts_col],
                            "direction": direction,
                            "size": size,
                ...
                if date_trades:
                    ...
                    date_pnls.append({
                        ...
                        "avg_size": np.mean([t["size"] for t in date_trades]),
                    })
                    all_trades.extend(date_trades)
                else:
                    date_pnls.append({"date": date_str, "n_trades": 0, "pnl_sum": 0, "win_rate": 0, "avg_size": 0})
            ...
            avg_size = np.mean([d["avg_size"] for d in date_pnls if d["avg_size"] > 0]) if any(d["avg_size"] > 0 for d in date_pnls) else 0
            ...
            all_results[(sizing_name, fee_name)] = {
    ...
    print()
    print(f"{'Sizing':<28} {'Fee':<18} {'Trades':<8} {'AvgSize':<10} {'Daily':<11} {'Sharpe':<9} {'Max DD':<10}")
    print("-" * 100)

    for sizing_name in SIZING_STRATEGIES:
        for fee_name in FEE_SCENARIOS:
            r = all_results[(sizing_name, fee_name)]
            print(f"{sizing_name:<28} {fee_name:<18} {r['total_trades']:<8} {r['avg_size']:<10.3f} {r['daily_avg']:<+11.3f}% {r['sharpe']:<9.2f} {r['max_dd']:<10.3f}%")
        print()

    print()
    print("=" * 80)
    print("BEST SIZING PER FEE SCENARIO")
    print("=" * 80)

    for fee_name in FEE_SCENARIOS:
        best_sharpe = max(SIZING_STRATEGIES.keys(), key=lambda s: all_results[(s, fee_name)]["sharpe"])
        best_avg = max(SIZING_STRATEGIES.keys(), key=lambda s: all_results[(s, fee_name)]["daily_avg"])

        print(f"\n{fee_name}:")
        print(f"  Best Sharpe: {best_sharpe} (Sharpe {all_results[(best_sharpe, fee_name)]['sharpe']:.2f}, daily {all_results[(best_sharpe, fee_name)]['daily_avg']:+.3f}%)")
        print(f"  Best Daily:  {best_avg} (daily {all_results[(best_avg, fee_name)]['daily_avg']:+.3f}%, Sharpe {all_results[(best_avg, fee_name)]['sharpe']:.2f})")

    print()
    print("=" * 80)
    print("PER-DATE BREAKDOWN - Best Maker Strategy")
    print("=" * 80)

    best_maker = max(SIZING_STRATEGIES.keys(), key=lambda s: all_results[(s, "Maker (-0.05%)")]["sharpe"])
    print(f"\nBest Maker by Sharpe: {best_maker}")
    for d in all_results[(best_maker, "Maker (-0.05%)")]["date_pnls"]:
        print(f"  {d['date']}: trades={d['n_trades']:<3} pnl={d['pnl_sum']:+.3f}% size={d['avg_size']:.2f} winrate={d['win_rate']:.3f}")
    ...
    print("=" * 80)
    print()
    print(f"{'Strategy':<28} {'Maker':<28} {'Mixed':<28}")
    print("-" * 90)
    for sizing_name in SIZING_STRATEGIES:
        maker_r = all_results[(sizing_name, "Maker (-0.05%)")]
        mixed_r = all_results[(sizing_name, "Mixed (0.03%)")]
        maker_str = f"{maker_r['daily_avg']:+.2f}% (Sh={maker_r['sharpe']:.2f})"
        mixed_str = f"{mixed_r['daily_avg']:+.2f}% (Sh={mixed_r['sharpe']:.2f})"
        print(f"{sizing_name:<28} {maker_str:<28} {mixed_str:<28}")

    print()
    print("Decision guide:")
    print("  - Sharpe 1.5+ in Maker -> 시도 14 (Maker fill)")
    print("  - Sharpe 1.0-1.5 in Maker -> 시도 8 (Regime) or 16 (Asym exit)")
    print("  - Sharpe < 1.0 -> sizing tweak more")
